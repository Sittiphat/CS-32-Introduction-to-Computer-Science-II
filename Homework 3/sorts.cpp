...
/*    // TODO: You implement this.  Using the same criteria as compareSensor,
  ...
  return compareSensor(*lhs, *rhs); // Using above function as helper function but for pointers
}
...
/*    // TODO: Using the insertion sort algorithm (pp. 332-333), sort s
  ...
  // Chooses size of elements we are dealing with
  for (int i = 2; i <= s.size(); i++) {
    // Last element stored on what we are dealing with
    Sensor last_num = s[i-1];

    // Index before last number
    int j = i - 2;
    // Shift elements one over right until last element in order
    while (j >= 0 && comp(last_num, s[j])) { // last_num > arr[j] (great->less) instead we use comp() fix: j = -1 access comp()
      // cerr << "line 203: " << i << '\t' <<  j << endl;
      s[j+1] = s[j];
      j--;
    }
    // Putting last element in proper order position after shift
    // cerr << "line 208: " << i << '\t' <<  j << endl;
    s[j+1] = last_num;
  }
}
...
int main()
{
      ...
      // destruction time.
    {
     // vector<Sensor> auxSensors(sensors);
      ...
      vector<Sensor> auxSensors(sensors);
      vector<Sensor*> auxSensorsPtr; // Declaring vector of pointers of above vector

      for (size_t i = 0; i < auxSensors.size(); i++) {
        auxSensorsPtr.push_back(&auxSensors[i]); // Saving the address of auxSensors
      }

      sort(auxSensorsPtr.begin(), auxSensorsPtr.end(), compareSensorPtr); // Sorting auxSensorPtr using algorithms library

      for (size_t i = 0; i < auxSensorsPtr.size(); i++) { // As instructed, dereference the vector of pointers to replace each Sensor in sensors from auxSensors
